...
def generate_layout_constrained_solution(
    dataspace_deps: List[List['Dataspace']],
    layer_dataspace_layouts: Dict[int, 'LayerDataspaceLayouts'],
    layer_dataspaces: Dict[int, 'LayerDataspaces'],
    base_problem_path: str = "/home/ubuntu/squareloop/benchmarks/script/crosslayer_search/test/resnet18",
    output_base_path: str = "/home/ubuntu/squareloop/benchmarks/layout_constrained_resnet18"
) -> Dict[int, 'LayerDataspaceLayouts']:
    """
    Generate layout-constrained solution where all data within dependency groups share the same layout.
    This eliminates rehashing overhead by forcing consumers to use producer layouts.

    Args:
        dataspace_deps: List of dependency groups containing dataspaces
        layer_dataspace_layouts: Current optimal layouts for each layer
        layer_dataspaces: Layer dataspace definitions  
        base_problem_path: Path to original layer problem definition files
        output_base_path: Base path for output folder structure

    Returns:
        Dict mapping layer_id to updated LayerDataspaceLayouts with constrained layouts
    """
    # Create output directory
    os.makedirs(output_base_path, exist_ok=True)
    
    # Define rank mapping between outputs and inputs
    output_to_input_mapping = {'N': 'N', 'L': 'V', 'P': 'H', 'Q': 'W'}
    input_to_output_mapping = {v: k for k, v in output_to_input_mapping.items()}
    
    # Create a copy of current layouts to modify
    constrained_layouts = {}
    for layer_id, layout in layer_dataspace_layouts.items():
        constrained_layouts[layer_id] = LayerDataspaceLayouts(
            layer_id=layer_id,
            inputs_layouts=[],
            weights_layouts=layout.weights_layouts.copy(),  # Weights remain unchanged
            outputs_layouts=[],
            targets=layout.targets.copy()
        )
    
    print(f"Generating layout-constrained solution...")
    print(f"Processing {len(dataspace_deps)} dependency groups...")
    
    # Process each dependency group
    for group_idx, dep_group in enumerate(dataspace_deps, 1):
        print(f"\nProcessing Dependency Group {group_idx}:")
        print(f"  Dataspaces: {[str(ds) for ds in dep_group]}")
        
        # Find the producer (output) and consumers (inputs) in this group
        producers = [ds for ds in dep_group if ds.dataspace_type == "Outputs"]
        consumers = [ds for ds in dep_group if ds.dataspace_type == "Inputs"]
        
        if not producers:
            print(f"  Warning: No producers found in group {group_idx}")
            continue
            
        if not consumers:
            print(f"  Warning: No consumers found in group {group_idx}")
            continue
            
        # Use the first producer as the canonical layout source
        producer = producers[0]
        producer_layer_id = producer.layer_id
        
        print(f"  Producer: {producer}")
        print(f"  Consumers: {[str(c) for c in consumers]}")
        
        # Get producer's output layouts from original optimal solution
        if producer_layer_id not in layer_dataspace_layouts:
            print(f"  Warning: No layout found for producer layer {producer_layer_id}")
            continue
            
        producer_layouts = layer_dataspace_layouts[producer_layer_id].outputs_layouts
        
        # Apply producer's layout to all consumers in the group
        for consumer in consumers:
            consumer_layer_id = consumer.layer_id
            
            # Convert producer's output layouts to consumer's input layouts
            adapted_layouts = []
            
            for producer_layout in producer_layouts:
                # Create adapted layout for consumer input
                adapted_factors = {}
                adapted_permutation = ""
                
                # Map factors and permutation from output to input ranks
                for rank, factor in producer_layout.factors.items():
                    if rank in output_to_input_mapping:
                        input_rank = output_to_input_mapping[rank]
                        adapted_factors[input_rank] = factor
                    else:
                        # Keep ranks that don't need mapping
                        adapted_factors[rank] = factor
                
                # Map permutation string
                for char in producer_layout.permutation:
                    if char in output_to_input_mapping:
                        adapted_permutation += output_to_input_mapping[char]
                    else:
                        adapted_permutation += char
                
                adapted_layout = DataspaceLayoutNest(
                    dataspace_name="Inputs",
                    target=producer_layout.target,
                    layout_type=producer_layout.layout_type,
                    factors=adapted_factors,
                    permutation=adapted_permutation,
                    ranks=list(adapted_permutation) if adapted_permutation else []
                )
                adapted_layouts.append(adapted_layout)
            
            # Update constrained layouts for this consumer
            if consumer_layer_id not in constrained_layouts:
                constrained_layouts[consumer_layer_id] = LayerDataspaceLayouts(
                    layer_id=consumer_layer_id,
                    inputs_layouts=[],
                    weights_layouts=[],
                    outputs_layouts=[],
                    targets=[]
            ...
            constrained_layouts[consumer_layer_id].inputs_layouts = adapted_layouts
            
            print(f"    Updated consumer {consumer} with adapted producer layout")
        
        # Copy producer's output layouts to constrained solution
        if producer_layer_id in constrained_layouts:
            constrained_layouts[producer_layer_id].outputs_layouts = producer_layouts.copy()
    
    # For layers not in dependency groups, keep their original optimal layouts
    for layer_id, original_layout in layer_dataspace_layouts.items():
        if layer_id not in constrained_layouts:
            constrained_layouts[layer_id] = original_layout
        else:
            # Ensure weights and any missing layouts are preserved
            if not constrained_layouts[layer_id].weights_layouts:
                constrained_layouts[layer_id].weights_layouts = original_layout.weights_layouts.copy()
            if not constrained_layouts[layer_id].inputs_layouts:
                constrained_layouts[layer_id].inputs_layouts = original_layout.inputs_layouts.copy()
            if not constrained_layouts[layer_id].outputs_layouts:
                constrained_layouts[layer_id].outputs_layouts = original_layout.outputs_layouts.copy()
    
    print(f"\nGenerating output files...")
    
    # Generate layout.yaml and layer.yaml files for each layer
    for layer_id in sorted(constrained_layouts.keys()):
        layer_folder = os.path.join(output_base_path, f"resnet18_layer{layer_id}")
        os.makedirs(layer_folder, exist_ok=True)
        
        # Generate layout.yaml for this layer
        generate_constrained_layout_file(
            layer_id, 
            constrained_layouts[layer_id], 
            os.path.join(layer_folder, "layout.yaml")
        )
        
        # Copy original layer.yaml file
        original_layer_file = os.path.join(base_problem_path, f"resnet18_layer{layer_id}.yaml")
        target_layer_file = os.path.join(layer_folder, "layer.yaml")
        
        if os.path.exists(original_layer_file):
            shutil.copy2(original_layer_file, target_layer_file)
        else:
            print(f"  Warning: Original layer file not found: {original_layer_file}")
    
    print(f"Layout-constrained solution generated in: {output_base_path}")
    print(f"Generated files for {len(constrained_layouts)} layers")
    
    return constrained_layouts
...
def compare_layout_solutions(
    original_layouts: Dict[int, 'LayerDataspaceLayouts'],
    constrained_layouts: Dict[int, 'LayerDataspaceLayouts'],
    dataspace_deps: List[List['Dataspace']]
) -> None:
    """
    Compare original optimal layouts with layout-constrained solutions.

    Args:
        original_layouts: Original optimal layouts for each layer
        constrained_layouts: Layout-constrained layouts for each layer  
        dataspace_deps: Dependency groups for context
    """
    print("\n" + "="*60)
    print("LAYOUT SOLUTION COMPARISON")
    print("="*60)
    
    print(f"Comparing layouts for {len(original_layouts)} layers...")
    
    # Track changes by layer
    layers_with_changes = []
    total_layout_changes = 0
    
    for layer_id in sorted(original_layouts.keys()):
        if layer_id not in constrained_layouts:
            continue
            
        original = original_layouts[layer_id]
        constrained = constrained_layouts[layer_id]
        
        # Compare inputs layouts
        input_changes = compare_layout_nests(
            original.inputs_layouts, 
            constrained.inputs_layouts, 
            f"Layer {layer_id} Inputs"
        )
        
        # Compare outputs layouts  
        output_changes = compare_layout_nests(
            original.outputs_layouts,
            constrained.outputs_layouts,
            f"Layer {layer_id} Outputs"
        )
        
        layer_changes = input_changes + output_changes
        if layer_changes > 0:
            layers_with_changes.append(layer_id)
            total_layout_changes += layer_changes
    
    print(f"\nSummary:")
    print(f"  Layers with layout changes: {len(layers_with_changes)}")
    print(f"  Total layout changes: {total_layout_changes}")
    print(f"  Changed layers: {layers_with_changes}")
    
    # Show dependency group impact
    print(f"\nDependency Group Impact:")
    for group_idx, dep_group in enumerate(dataspace_deps, 1):
        group_layers = list(set(ds.layer_id for ds in dep_group))
        changed_layers_in_group = [l for l in group_layers if l in layers_with_changes]
        
        if changed_layers_in_group:
            print(f"  Group {group_idx}: {len(changed_layers_in_group)}/{len(group_layers)} layers changed")
            print(f"    Changed layers: {changed_layers_in_group}")
            print(f"    Dataspaces: {[str(ds) for ds in dep_group]}")
...
def run_layout_constrained_mapping_search(
    constrained_base_path: str = "/home/ubuntu/squareloop/benchmarks/layout_constrained_resnet18",
    arch_yaml: str = "/home/ubuntu/squareloop/benchmarks/secureloop-cross-eval/arch/baseline.yaml",
    arch_components: str = "/home/ubuntu/squareloop/benchmarks/secureloop-cross-eval/arch/components/*",
    mapper_yaml: str = "/home/ubuntu/squareloop/benchmarks/mapper/mapper_squarelooop.yaml",
    setup_script: str = "~/.setup.sh"
) -> Dict[int, int]:
    """
    Run layout-constrained mapping search using timeloop-mapper for all layers.
    
    Args:
        constrained_base_path: Base path to layout-constrained solution folders
        arch_yaml: Path to architecture YAML file
        arch_components: Path pattern to architecture component files
        mapper_yaml: Path to mapper configuration YAML file
        setup_script: Path to setup script for environment variables
        
    Returns:
        Dict mapping layer_id to compute latency cycles
    """
    import subprocess
    import glob
    
    print("\n" + "="*60)
    print("LAYOUT-CONSTRAINED MAPPING SEARCH")
    print("="*60)
    ...
    try:
        ...
        layer_dirs = []
        for item in os.listdir(constrained_base_path):
            item_path = os.path.join(constrained_base_path, item)
            if os.path.isdir(item_path) and item.startswith('resnet18_layer'):
                layer_dirs.append((item, item_path))
        
        # Sort by layer number
        layer_dirs.sort(key=lambda x: int(x[0].replace('resnet18_layer', '')))
        
        print(f"Found {len(layer_dirs)} layer directories to process:")
        for layer_name, _ in layer_dirs:
            print(f"  {layer_name}")
        
        # Process each layer
        for layer_name, layer_path in layer_dirs:
            # Extract layer number
            layer_match = re.search(r'resnet18_layer(\d+)', layer_name)
            if not layer_match:
            ...
            print(f"\nProcessing {layer_name} (Layer {layer_id}):")
            print(f"  Working directory: {layer_path}")
            
            # Check required files exist
            layer_yaml_path = os.path.join(layer_path, "layer.yaml")
            layout_yaml_path = os.path.join(layer_path, "layout.yaml")
            
            if not os.path.exists(layer_yaml_path):
                print(f"  Error: layer.yaml not found at {layer_yaml_path}")
                continue
                
            if not os.path.exists(layout_yaml_path):
                print(f"  Error: layout.yaml not found at {layout_yaml_path}")
                continue
            
            if not os.path.exists(arch_yaml):
                print(f"  Error: Architecture file not found at {arch_yaml}")
                continue
                
            if not os.path.exists(mapper_yaml):
                print(f"  Error: Mapper file not found at {mapper_yaml}")
                continue
            
            # Construct timeloop-mapper command
            # Command format: timeloop-mapper {layer.yaml} {layout.yaml} {arch.yaml} {mapper.yaml}
            cmd_parts = [
                "timeloop-mapper",
                "layer.yaml",           # Use relative paths since we're in the layer directory
                "layout.yaml", 
                arch_yaml,              # Use absolute path for arch
                arch_components,        # Architecture components
                mapper_yaml             # Use absolute path for mapper
            ]
            ...
            try:
                # Change to layer directory and run the command
                ...
                if result.returncode == 0:
                    print(f"  ✓ Timeloop-mapper completed successfully")
                    
                    # Extract cycles from stats file
                    stats_file_path = os.path.join(layer_path, "timeloop-mapper.stats.txt")
                    layer_cycles = extract_cycles_from_stats(stats_file_path)
                    
                    if layer_cycles is not None:
                        layer_latencies[layer_id] = layer_cycles
                        print(f"  ✓ Extracted cycles: {layer_cycles:,}")
                    else:
                        print(f"  ✗ Failed to extract cycles from stats file")
                        
                else:
                    print(f"  ✗ Timeloop-mapper failed with return code {result.returncode}")
                    if result.stderr:
                        print(f"    Error output: {result.stderr[:500]}...")  # Show first 500 chars
                        
            except subprocess.TimeoutExpired:
                print(f"  ✗ Timeloop-mapper timed out after 30 minutes")
        ...
        print(f"\nMapping search completed for {len(layer_latencies)}/{len(layer_dirs)} layers")
        
        return layer_latencies
        
    except Exception as e:
        print(f"Error in layout-constrained mapping search: {e}")
        return {}


def calculate_constrained_solution_latency(
    constrained_layer_latencies: Dict[int, int],
    network_latency: NetworkLatencyResult,
    original_comprehensive_latency: ComprehensiveLatencyResult
) -> None:
    """
    Calculate and compare the total latency of the layout-constrained solution.
    
    Args:
        constrained_layer_latencies: Latencies from constrained mapping search
        network_latency: Original network latency analysis (for reorganization overhead)
        original_comprehensive_latency: Original comprehensive latency analysis
    """
    print("\n" + "="*60)
    print("LAYOUT-CONSTRAINED SOLUTION LATENCY ANALYSIS")
    print("="*60)
    
    if not constrained_layer_latencies:
        print("No constrained layer latencies available for analysis")
        return
    
    # Calculate constrained solution metrics
    constrained_processing_latency = sum(constrained_layer_latencies.values())
    constrained_layout_reorganization_latency = network_latency.total_reorganization_overhead
    constrained_rehash_latency = 0  # No rehashing in constrained solution
    constrained_interlayer_memory_latency = max(constrained_layout_reorganization_latency, constrained_rehash_latency)
    constrained_total_latency = constrained_processing_latency + constrained_interlayer_memory_latency
    
    # Calculate efficiency metrics
    if constrained_total_latency > 0:
        constrained_processing_efficiency = constrained_processing_latency / constrained_total_latency
        constrained_memory_overhead_ratio = constrained_interlayer_memory_latency / constrained_total_latency
    else:
        constrained_processing_efficiency = 0.0
        constrained_memory_overhead_ratio = 0.0
    
    print(f"Layout-Constrained Solution Metrics:")
    print(f"  Processing Latency: {constrained_processing_latency:,} cycles")
    print(f"  Layout Reorganization Latency: {constrained_layout_reorganization_latency:,} cycles")
    print(f"  Rehash Latency: {constrained_rehash_latency:,} cycles (eliminated)")
    print(f"  Interlayer Memory Latency: {constrained_interlayer_memory_latency:,} cycles")
    print(f"  Total Latency: {constrained_total_latency:,} cycles")
    
    print(f"\nEfficiency Metrics:")
    print(f"  Processing Efficiency: {constrained_processing_efficiency:.4f} ({constrained_processing_efficiency*100:.2f}%)")
    print(f"  Memory Overhead: {constrained_memory_overhead_ratio:.4f} ({constrained_memory_overhead_ratio*100:.2f}%)")
    ...
    original_total = original_comprehensive_latency.total_latency
    original_processing = original_comprehensive_latency.processing_latency
    original_rehash = original_comprehensive_latency.rehash_latency
    
    print(f"Original Solution:")
    print(f"  Processing Latency: {original_processing:,} cycles")
    print(f"  Rehash Latency: {original_rehash:,} cycles")
    print(f"  Total Latency: {original_total:,} cycles")
    
    print(f"\nConstrained Solution:")
    print(f"  Processing Latency: {constrained_processing_latency:,} cycles")
    print(f"  Rehash Latency: {constrained_rehash_latency:,} cycles")
    print(f"  Total Latency: {constrained_total_latency:,} cycles")
    
    # Calculate differences
    processing_diff = constrained_processing_latency - original_processing
    total_diff = constrained_total_latency - original_total
    rehash_savings = original_rehash - constrained_rehash_latency
    
    print(f"\nLatency Differences:")
    print(f"  Processing Difference: {processing_diff:+,} cycles ({(processing_diff/original_processing)*100:+.2f}%)")
    print(f"  Rehash Savings: {rehash_savings:+,} cycles")
    print(f"  Total Difference: {total_diff:+,} cycles ({(total_diff/original_total)*100:+.2f}%)")
    
    # Determine which solution is better
    if total_diff < 0:
        print(f"\n🎉 Layout-Constrained Solution is BETTER by {abs(total_diff):,} cycles ({abs(total_diff/original_total)*100:.2f}%)")
    elif total_diff > 0:
        print(f"\n⚠️  Original Solution is better by {total_diff:,} cycles ({(total_diff/original_total)*100:.2f}%)")
    else:
        print(f"\n🤝 Both solutions have the same total latency")
    
    # Layer-by-layer comparison if we have original data
    if hasattr(original_comprehensive_latency, 'layer_latencies') and original_comprehensive_latency.layer_latencies:
        print(f"\nLayer-by-Layer Processing Latency Comparison:")
        for layer_id in sorted(set(constrained_layer_latencies.keys()) | set(original_comprehensive_latency.layer_latencies.keys())):
            original_cycles = original_comprehensive_latency.layer_latencies.get(layer_id, 0)
            constrained_cycles = constrained_layer_latencies.get(layer_id, 0)
            diff = constrained_cycles - original_cycles
            
            if original_cycles > 0:
                diff_pct = (diff / original_cycles) * 100
                print(f"  Layer {layer_id}: {original_cycles:,} → {constrained_cycles:,} ({diff:+,}, {diff_pct:+.1f}%)")
            else:
                print(f"  Layer {layer_id}: missing → {constrained_cycles:,}")
    
    print(f"\nSummary:")
    if rehash_savings > abs(processing_diff):
        print(f"  ✓ Rehashing elimination ({rehash_savings:,} cycles) outweighs processing overhead")
    elif abs(processing_diff) > rehash_savings:
        print(f"  ⚠️ Processing overhead ({abs(processing_diff):,} cycles) exceeds rehashing savings")
    else:
        print(f"  ⚖️ Processing changes and rehashing savings are balanced")
...
if __name__ == "__main__":
  ...
  print("="*60)
  base_path = "/home/ubuntu/squareloop/benchmarks/squareloop_resent18"
  ...
  # Determine which interlayer memory component is dominant
  if comprehensive_latency.layout_reorganization_latency > comprehensive_latency.rehash_latency:
    dominant_component = "Layout Reorganization"
    dominant_latency = comprehensive_latency.layout_reorganization_latency
    non_dominant_component = "Rehash"
    non_dominant_latency = comprehensive_latency.rehash_latency
  else:
    dominant_component = "Rehash"
    dominant_latency = comprehensive_latency.rehash_latency
    non_dominant_component = "Layout Reorganization"
    non_dominant_latency = comprehensive_latency.layout_reorganization_latency

  print(f"\nInterlayer Memory Breakdown:")
  print(f"  Dominant Component: {dominant_component} ({dominant_latency:,} cycles)")
  print(f"  Non-Dominant Component: {non_dominant_component} ({non_dominant_latency:,} cycles)")

  if dominant_latency > 0:
    dominance_ratio = non_dominant_latency / dominant_latency
    print(f"  Dominance Ratio: {dominance_ratio:.4f} ({non_dominant_component} is {dominance_ratio*100:.2f}% of {dominant_component})")

  print(f"\nEfficiency Metrics:")
  print(f"  Compute Efficiency: {(comprehensive_latency.processing_latency/comprehensive_latency.total_latency)*100:.2f}% (processing / total)")
  print(f"  Memory Overhead: {(comprehensive_latency.interlayer_memory_latency/comprehensive_latency.total_latency)*100:.2f}% (interlayer memory / total)")

  # Performance summary
  print(f"\nPerformance Summary:")
  print(f"  Total Execution Time: {comprehensive_latency.total_latency:,} cycles")
  print(f"  Productive Computation: {comprehensive_latency.processing_latency:,} cycles ({comprehensive_latency.processing_efficiency*100:.2f}%)")
  print(f"  Memory Management Overhead: {comprehensive_latency.interlayer_memory_latency:,} cycles ({(1-comprehensive_latency.processing_efficiency)*100:.2f}%)")

  # Generate layout-constrained solution to eliminate rehashing overhead
  print("\n" + "="*60)
  print("LAYOUT-CONSTRAINED SOLUTION GENERATION")
  print("="*60)
  
  # Generate the constrained solution
  constrained_layouts = generate_layout_constrained_solution(
      dataspace_deps=dataspace_deps,
      layer_dataspace_layouts=layer_dataspace_layouts,
      layer_dataspaces=layer_dataspaces,
      base_problem_path="/home/ubuntu/squareloop/benchmarks/script/crosslayer_search/test/resnet18",
      output_base_path="/home/ubuntu/squareloop/benchmarks/layout_constrained_resnet18"
  )
  
  # Compare the two solutions
  compare_layout_solutions(
      original_layouts=layer_dataspace_layouts,
      constrained_layouts=constrained_layouts,
      dataspace_deps=dataspace_deps
  )
  
  print(f"\nLayout-Constrained Solution Benefits:")
  print(f"  ✓ Eliminates rehashing latency ({sum(group_rehash_latencies.values()):,} cycles)")
  print(f"  ✓ Reduces interlayer memory overhead")
  print(f"  ✓ Simplifies memory management")
  print(f"  ✗ May have suboptimal compute performance per layer")
  
  # Calculate potential savings
  print(f"\nPotential Latency Savings:")
  print(f"  Rehash Latency Eliminated: {sum(group_rehash_latencies.values()):,} cycles")
  print(f"  Layout Reorganization Still Required: {network_latency.total_reorganization_overhead:,} cycles")
  
  # Estimate new total latency (processing + reorganization, no rehashing)
  constrained_total_latency = comprehensive_latency.processing_latency + network_latency.total_reorganization_overhead
  latency_savings = comprehensive_latency.total_latency - constrained_total_latency
  
  print(f"  Original Total Latency: {comprehensive_latency.total_latency:,} cycles")
  print(f"  Constrained Total Latency (estimated): {constrained_total_latency:,} cycles")
  print(f"  Estimated Savings: {latency_savings:,} cycles ({(latency_savings/comprehensive_latency.total_latency)*100:.2f}%)")
  
  print(f"\nNext Steps:")
  print(f"  1. Run timeloop-mapper with the generated constrained layouts")
  print(f"  2. Measure actual compute latency for each layer")
  print(f"  3. Compare total latency: constrained vs optimal+rehashing")
  print(f"  4. Analyze the trade-off between compute efficiency and memory overhead")

  # Run layout-constrained mapping search
  print("\n" + "="*60)
  print("RUNNING LAYOUT-CONSTRAINED MAPPING SEARCH")
  print("="*60)
  
  constrained_layer_latencies = run_layout_constrained_mapping_search()
  
  # Calculate and compare constrained solution latency
  if constrained_layer_latencies:
    calculate_constrained_solution_latency(
        constrained_layer_latencies=constrained_layer_latencies,
        network_latency=network_latency,
        original_comprehensive_latency=comprehensive_latency
    )
  else:
    print("⚠️ No constrained layer latencies were obtained. Skipping latency comparison.")
    print("Please check if timeloop-mapper runs are successful and stats files are generated.")
