...
timeloop_mapper = timeloop_path + 'timeloop-model'

exp1_dir = '/home/ubuntu/repo/squareloop/benchmarks/exp_sec/'
...
def extract_energy_latency(output):
    energy = None
    cycles = None
    with open('timeloop-model.stats.txt', 'r') as f:
        lines = f.readlines()
...
def run_mapper(arch, model, layer):
    ...
    mapping_file = exp1_dir + 'designs/' + arch + '/' + model + '/evaluation/' + model + '/layer' + str(layer) + '/mapping.yaml'
    layout_file = exp1_dir + 'designs/' + arch + '/' + model + '/evaluation/' + model + '/layer' + str(layer) + '/layout.yaml'

    mapper_command = timeloop_ld_path + ' ' + timeloop_mapper + ' ' + arch_file + ' ' + workload_file + ' ' + crypto_file + ' ' + layout_file + ' ' + mapping_file
